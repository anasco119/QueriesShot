    ...
    try:
        ...
        # معالجة حسب النية
            ...
            if intent == "1":  # استفسار عام
                ...
                prompt = f"""
أنت مدرس لغة إنجليزية محترف، ومهمتك هي مساعدة الطالب {user_name} على الانتقال من التفكير الحرفي (كالترجمة من العربية) إلى التفكير بأسلوب أمريكي طبيعي عند الكتابة أو التحدث بالإنجليزية.

استلمت منه هذه القطعة، ومهمتك تقديم تقييم لغوي مهني ولطيف، يركز على تحسين العبارات والمفردات التي تبدو مترجمة أو غير طبيعية. لا تستهدف التشدق أو جعل النص متكلفًا، بل اجعله فقط أكثر أصالة وسلاسة.
قيّم القطعة كمعلم لغات يساعد طالبًا في التدرّب على التفكير كالناطقين الأصليين. لا تستخدم معيارًا صارمًا كأنك تصحح لمتحدث محترف، بل راعِ أن الطالب ما زال يتعلم ويتطور. إذا كانت الجمل واضحة وطبيعية ومفهومة، اعتبرها جيدة حتى لو كانت بسيطة.

اتبع دائمًا هذا النسق:
