...
اكتب كل الشرح بالعربية الا جزئية الخطأ كم هي. """
            ...
            # كتم العضو لمدة 10 دقائق إذا تكررت المخالفة
                ...
                message = update.message.text
                user_name = get_user_name(update)
                prompt = f"""اكتب شرح معنى كلمة إنجليزية لهذا الطالب، باتباع تنسيق ثابت وجذاب بصريًا.  
يُستخدم هذا الرد في شات مع طلاب يتعلمون الإنجليزية، لذا يجب أن يكون مختصرًا، واضحًا، ومُحفّزًا.

ابدأ بتحية قصيرة باسم الطالب، ثم اكتب النسق الثابت 
...
**تعليمات إضافية:**
1. استخدم لغة واضحة كما في المثال .  
