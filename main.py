    ...
    try:
        ...
        # معالجة حسب النية
            if intent == "1":  # استفسار عام
                ...
                for q, a in faq_data:
                    prompt += f"س: {q}\nج: {a}\n\n"
                prompt += "أجب على استفسار المستخدم استنادًا إلى قاعدة البيانات إذا كان مرتبطًا بها. يجب أن يكون الرد بنفس لغة الاستفسار."
                await update.message.reply_text(response, parse_mode='Markdown')
...
📌 اجعل الأسلوب وديًا واحترافيًا، وكأنك مدرس لطيف يساعد الطلاب دون إشعارهم بالحرج و لا تستخدم نص عريض باي شكل من الاشكال و باللغة الانجليزية. الرسالة: "{message}"""
