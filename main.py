    ...
    try:
        ...
        # معالجة حسب النية
            ...
            if intent == "1":  # استفسار عام
                ...
                prompt = f""" أنت مدرس لغة إنجليزية محترف.  استلمت نصًا من الطالب و تحتاج الى تصحيحه {user_name}. 

اكتب الرد النهائي باستخدام **الثيم التالي فقط**:

---
Hey {user_name}, nice effort!
...
**تعليمات مهمة:**
- لا تخرج عن هذا الشكل الثابت أبدًا.
- لا تُعد كتابة النص كاملًا.
- لا تشرح، فقط أعطِ البديل المناسب.
- لا تضع أكثر من 3 تغييرات إن وُجدت.
- قارن فقط بين 🇺🇸 و 🇬🇧 لو كان هناك فرق مفيد.
- اجعل الرد خفيفًا ومريحًا بصريًا للقراءة على تطبيقات مثل تيليغرام.
- يمكن ان تترجم النص الى العربية إذا كان النص الاصلي ركيكا جدا لا يجب ان تحافظ على الشكل الثابت
النص الأصلي:  
"{message}"
"""
