    ...
    try:
        ...
        # تطبيق القيود المطبقة في المجموعة
            ...
            intent_prompt = f"""حدد نية الرسالة التالية من الخيارات التالية فقط:
                ...
                6. السؤال مباشر عن معنى كلمة او عبارة بالانجليزية او مثل شائع
        ...
        # معالجة حسب النية
            ...
            if intent == "1":  # استفسار عام
                ...
                prompt = f"""
أنت مدرس لغة إنجليزية محترف. استلمت قطعة مكتوبة من الطالب {user_name}.  
مهمتك هي تقديم تقييم لغوي مختصر ومهني، مع تصحيح 
للمفردات او العبارات
باتباع النسق الثابت التالي **دائمًا**:

---

مرحباً {user_name} ، القطعة ممتازة و لغتها سليمة من الاخطاء؛ هذا عمل جميل، دعني أذكر بعض النقاط البسيطة لتساعدك في إكتساب البلاغة في اللغة : 
...
Try to use that in your sentences
...
- قارن فقط بين الإنجليزية الأمريكية والبريطانية (إن وُجد فرق يستحق الذكر).
- اجعل الرد كله لا يتجاوز 10 أسطر في تيليغرام أو تطبيقات المراسلة المشابهة.
- الأسلوب يجب أن يكون لبقاً، محفزاً، وخفيفاً دون مبالغة.
-  إذا كانت القطعة المكتوبة فصيحة و إحترافية بشكل واضح اكتب النسق العام بالانجليزية، غير ذلك: ارسل الرد بالعربية 
- الحكم الأولي على القطعة يجب ان يكون حقيقي بناء على مضمون القطعة.
- يمكنك تعديل كلمات المجاملة اعلاه بأي أسلوب لبق مع المحافظة على النسق العام.
- يمكن تعديل نص البداية مع الحفاظ على مضمونه
- النصيحة الاخيرة يمكنك تعديلها لتكون مناسبة للتصحيحات
...
ابدأ بتحية قصيرة باسم الطالب، ثم استخدم الرموز التعبيرية والعناوين الجانبية بخط عريض، مع الحفاظ على البساطة والوضوح.  
اخفِ الترجمة العربية باستخدام تنسيق spoiler (علامتي || قبل وبعد الترجمة).

**مثال على النسق الثابت:**

Great pick, {user_name}!  
📌 *resilient*  
🗣️ **Meaning:** able to recover quickly from difficulties  
🪄 **بالعربي:** ||مرِن وقوي أمام الصعوبات||  
✏️ **Example:** She stayed resilient after losing the game.

🌱 Keep learning — every word makes a difference!

**تعليمات إضافية:**
1. استخدم تنسيق MarkdownV2 فقط.  
2. التزم بالنسق تمامًا كما في المثال.  
...
اسم الطالب = "{user_name}" أذكره في النسق إذا كان إسما حقيقيا غير رمز
"""
                print("\n===== DEBUG: نية 6 =====")
